...
export default function ProjectOverview({ selectedProject, onSave, handleProjectSelect }) {
    // const newProjectDialog = useRef();
    const isProjectSelected = !!selectedProject;
    const isDraftProject = selectedProject?.isDraft;
    ...
    return (
        <>
            ...
            {!isProjectSelected ? (<div className="mt-24 text-center w-2/3">
                ...
                <button onClick={handleNewProject} className="px-4 py-2 text-xs md:text-base rounded-md bg-stone-700 text-stone-400 hover:bg-stone-600 hover:text-stone-100">
                    Create New Project
                </button>

            </div>) : null}

            {isProjectSelected && isDraftProject ?
                <EditProject project={selectedProject} /> :
                null
            }

            {isProjectSelected && !isDraftProject ? <ProjectDetails project={selectedProject} /> : null}
        </>
